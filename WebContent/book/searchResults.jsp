    ...
    <table border="1">
    	<tr><th>ID</th><th>ISBN番号</th><th>資料名</th><th>分類コード</th><th>著者</th><th>出版社</th><th>出版日</th><th>削除</th><th>変更</th><th>予約</th></tr>
      	<c:forEach books="${books}" var ="book">
      		<tr><td>"${book.id}"</td><td>"${book.isbn}"</td><td>"${book.name}"</td><td>"${book.classId}"</td><td>"${book.author}"</td><td>"${book.publisher}"</td><td>"${book.releaseDate}"</td><input type="submit" value="削除"></th><th><input type="submit" value="変更"></th><th><input type="submit" value="予約"></th></tr>
